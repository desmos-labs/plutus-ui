 ...
 */
function ProfileSearchResults({...props}: React.HTMLAttributes<HTMLDivElement>) {
    ...
    >
      ...
      {state.searchResults.length == 0 &&
        <div className="p-2">
          ...
          <p className="mt-1">
            Looks like this use has not yet created a Desmos Profile and connected it with their social accounts yet
          </p>
        </div>
      }

      {state.searchResults.map((link) => {
        return <ProfileSearchResultItem link={link}/>
      })}
    </div>
  );
